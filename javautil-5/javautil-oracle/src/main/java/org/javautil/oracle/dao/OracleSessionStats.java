...
public class OracleSessionStats {
	...
	private static final String selectText = "" + "select  ss.sid, " + "    ss.statistic#," + "    ss.value "
			+ "from sys.v_$sesstat ss " + " where ss.value > 0";
	...
	public static OracleSessionStatSet getForMySession(final OracleConnection conn) throws SQLException {
		final int sid = OracleConnectionHelper.getSid(conn);
		return getForSession(conn, sid);
	}

	public static OracleSessionStatSet getForSession(final Connection conn, final int sid) throws SQLException {
	...
	public static void getRow(final ResultSet rset, final OracleSessionStat row) throws java.sql.SQLException {
