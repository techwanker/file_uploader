...
public class OraclePrimaryKeys {
	...
	private static final String query = "select ac.constraint_name, aic.column_name, aic.column_position "
			+ "from all_ind_columns aic, all_constraints ac " + "where aic.index_name = ac.constraint_name and "
			+ "       ac.constraint_type = 'P' and " + "      aic.index_owner = ac.owner and"
			+ "      aic.index_owner = :owner and " + "      ac.owner = :owner and "
			+ "      aic.table_name = :table_name and " + "      ac.table_name = :table_name";

	public static PrimaryKey getPrimaryKey(Connection conn, String schemaName, String tableName) throws SQLException {
		if (conn == null) {
