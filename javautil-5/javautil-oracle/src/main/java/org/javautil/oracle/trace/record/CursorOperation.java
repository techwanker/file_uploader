 ...
 * 
 * c CPU time (100th's of a second in Oracle7 ,8 and 9). e Elapsed time (100th's
 * of a second Oracle7, 8 Microseconds in Oracle 9 onwards). p Number of
 * physical reads. cr Number of buffers retrieved for CR reads. cu Number of
 * buffers retrieved in current mode. mis Cursor missed in the cache. r Number
 * of rows processed. dep Recursive call depth (0 = user SQL, >0 = recursive).
...
public abstract class CursorOperation extends AbstractCursorTraceEvent implements Record {
	private static Logger logger = Logger.getLogger(CursorOperation.class.getName());
	...
	*/
	 ...
	 * time (100th's of a second in Oracle7 ,8 and 9).
	 */
	private int cpu;
	/**
	...
	/**
	 * Number of blocks retrieved from cache in current mode. In trace file as
	 * "cu".
	 */
