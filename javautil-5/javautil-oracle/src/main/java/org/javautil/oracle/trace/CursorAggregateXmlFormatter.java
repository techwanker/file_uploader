...
public class CursorAggregateXmlFormatter implements CursorFormatter {
	...
	private final CursorAggregate cursor;
	 ...
	 */
	private final boolean sparseAggregateStatistics = true;

	static {
		events.addEvent(LOG_NO_AGGREGATION);
	}

	public CursorAggregateXmlFormatter(final CursorAggregate cursor) {
		this.cursor = cursor;
	}
	...
	public Element asElement() {
		...
		// root.addAttribute("cursorId",
		// Integer.toString(cursor.getCursorId()));
		// formatOperations();
		// formatStats();
	 ...
	 * 
	 *            If true timestamps for the first and last time the operation
	 *            was encountered in the trace file is reported.
	 */
	...
	private void formatOperations() {
		...
		// addToRoot(formatOperationAggregation(cursor.getParseAggregation(),
		// "parse"));
		// } else {
		// if (events.exists(LOG_NO_AGGREGATION)) {
		// logger.warn("cursor " + cursor.getCursorId()
		// + " has no parse aggregation");
		// }
		// }
		//
		// if (cursor.getExecAggregation() != null) {
		// addToRoot(formatOperationAggregation(cursor.getExecAggregation(),
		// "exec"));
		// } else {
		// if (events.exists(LOG_NO_AGGREGATION)) {
		// logger.warn("cursor " + cursor.getCursorId()
		// + " has no exec aggregation");
		// }
		// }
		//
		// if (cursor.getFetchAggregation() != null) {
		// addToRoot(formatOperationAggregation(cursor.getFetchAggregation(),
		// "fetches"));
		// } else {
		// if (events.exists(LOG_NO_AGGREGATION)) {
		// logger.warn("cursor " + cursor.getCursorId()
		// + " has no fetch aggregation");
		// }
		// }
	}
	...
	private void formatStats() {
		...
		for (final Stat stat : stats) {
			...
			addAttribute(statEl, "cr", stat.getConsistentReads());
			addAttribute(statEl, "obj", stat.getObjectNumber());
		}
	}

	// private boolean hasRecursiveOperations() {
	// final boolean returnValue = cursor.getRecursiveFetchAggregation() != null
	// || cursor.getRecursiveExecAggregation() != null
	// || cursor.getRecursiveParseAggregation() != null
	// || cursor.getRecursiveUnmapAggregation() != null;
	// return returnValue;
	// }
}
