...
public class OracleForeignKeyDAO extends JDBCForeignKeys implements ForeignKeys {
	...
	String columns = "select " + " ac.OWNER, " + " ac.CONSTRAINT_NAME, " + //
			...
			"       ac.table_name like upper(:table_name) and " + "       ac.owner like upper(:owner)";
	...
	public OracleForeignKeyDAO(Connection conn, String schemaName, String tableName, List<OracleConstraint> constraints)
			throws SQLException {
		if (conn == null) {
		...
		while (rset.next()) {
			OracleConstraintsDAO.getImportedKeyConstraints(constraints, schemaName, tableName);
			new ForeignKeyImpl();
		}
	}
