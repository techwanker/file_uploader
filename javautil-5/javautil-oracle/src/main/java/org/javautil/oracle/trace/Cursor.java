...
public class Cursor {
	...
	private static Logger logger = Logger.getLogger(Cursor.class.getName());
	...
	/**
	 * Assigned strictly increasing number.
	 */
	...
	public Cursor(final int cursorId) {
		if (cursorId < 1) {
			throw new IllegalArgumentException("cursor id must be > 0 ");
		}

		this.cursorId = cursorId;
		setSqlId(Integer.toString(cursorId));
		if (logConstructors) {
	...
	public Cursor(final Parsing p, final int cursorId) {
		...
		this.cursorId = cursorId;
		setCursorNumber(p.getCursorNumber());
		parsing = p;
		setSqlId(parsing.getSqlid());
		if (logConstructors) {
	...
	/**
	 * TODO this should never be called with a Parsing record.
	 * 
	 * @param rec
	 */
	public void add(final CursorRecord rec) {
		if (events.exists(LOG_ADD_EVENT)) {
			logger.info("received " + rec + "\n");
			logger.info(" before:\n" + toString());
		}
		final RecordType r = rec.getRecordType();
		if (r.equals(RecordType.PARSING)) {
			logger.warn("This is bogus, should not be called with a parsing record");
		}
		if (r == null) {
		...
		setCursorNumber(rec.getCursorNumber());
		if (!aggregateOnly) {
			addRecord(rec);
		}
		aggregate(rec);
	 ...
	 * Aggregation policy.
	 * 
	 * If true (default) operations, (parse, exec, fetch) are aggregated and the
	 * individual trace records are not stored.
	 * 
	 * @param value
	 */
	...
	private void addRecord(final CursorRecord rec) {
		final RecordType r = rec.getRecordType();
		switch (r) {
		case EXEC:
			add((Exec) rec);
	...
	private void aggregate(final CursorRecord rec) {
		final RecordType r = rec.getRecordType();
		switch (r) {
		case EXEC:
		case PARSE:
	...
	public Integer getCursorNumber() {
		return cursorNumber;
	}

	public void setCursorNumber(Integer cursorNumber) {
