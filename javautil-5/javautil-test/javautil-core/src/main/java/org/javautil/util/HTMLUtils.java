...
public class HTMLUtils {
	...
	public static void setStyleAttribute(final Element element, final Map<String, String> values) {
		ElementHelper.setAttribute(element, "style", ';', ':', values);
	}

	public static void setStyleAttribute(final Element element, final String name, final String value) {
	...
	public static String asStylesheetTag(final Resource resource) {
		...
		if (ClassPathResource.class.isAssignableFrom(resource.getClass())
				|| FileSystemResource.class.isAssignableFrom(resource.getClass())) {
			...
			try {
				...
				throw new RuntimeException("error while reading resource: " + resource.getDescription(), e);
			} finally {
				if (input != null) {
					try {
						input.close();
					} catch (final Exception e) {
						throw new RuntimeException("error while close resource: " + resource.getDescription(), e);
					}
				}
			}
			s.append("</style>");
			...
			try {
				...
				throw new RuntimeException("error while getting resource uri: " + resource.getDescription(), e);
			}
			s.append("/>");
		}
		return s.toString();
