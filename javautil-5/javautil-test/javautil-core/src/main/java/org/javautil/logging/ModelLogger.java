...
public class ModelLogger {
	...
	public void trace(final String modelName, final Map<String, ? extends Object> model) {
		if (logger.isTraceEnabled()) {
			logger.trace(buildModelString(modelName, model));
		}
	}

	public void debug(final String modelName, final Map<String, ? extends Object> model) {
		if (logger.isDebugEnabled()) {
			logger.debug(buildModelString(modelName, model));
		}
	}

	public void warn(final String modelName, final Map<String, ? extends Object> model) {
		if (logger.isWarnEnabled()) {
			logger.warn(buildModelString(modelName, model));
		}
	}

	public void info(final String modelName, final Map<String, ? extends Object> model) {
		if (logger.isInfoEnabled()) {
			logger.info(buildModelString(modelName, model));
		}
	}

	public void error(final String modelName, final Map<String, ? extends Object> model) {
		if (logger.isErrorEnabled()) {
			logger.error(buildModelString(modelName, model));
		}
	}

	public void fatal(final String modelName, final Map<String, ? extends Object> model) {
		if (logger.isFatalEnabled()) {
			logger.fatal(buildModelString(modelName, model));
		}
	}

	protected String buildModelString(final String modelName, final Map<String, ? extends Object> model) {
	...
	protected void logModelObject(final Object modelObject, final StringBuilder buffy, final int depth) {
		if (modelObject == null) {
			buffy.append("null");
		...
		} else if (modelObject.getClass().getPackage().getName().startsWith("java")) {
			...
			for (final Method method : methods) {
				if (Modifier.isPublic(method.getModifiers()) && method.getName().startsWith("get")
						&& method.getDeclaringClass().equals(modelObject.getClass())) {
					final String modelProperty = Character.toLowerCase(method.getName().charAt(3))
	...
	protected void logModelMap(final Map<Object, Object> map, final StringBuilder buffy, final int depth) {
		Iterator<Object> iterator = map.keySet().iterator();
		buffy.append("{");
		int printedCount = 0;
		while (iterator != null && iterator.hasNext()) {
			if (!Events.isRegistered("quickmodel") || printedCount < maxCollectionSize) {
				if (printedCount != 0) {
					buffy.append(", ");
				...
				buffy.append("..." + (map.size() - 5) + " more, " + map.size() + " total");
				iterator = null;
			}
		}
		buffy.append("}");
	...
	protected void logModelCollection(final Collection<Object> collection, final StringBuilder buffy, final int depth) {
		...
		while (iterator != null && iterator.hasNext()) {
			if (!Events.isRegistered("quickmodel") || printedCount < maxCollectionSize) {
				...
				buffy.append("..." + (collection.size() - 5) + " more, " + collection.size() + " total");
				iterator = null;
			}
		}
		buffy.append("]");
