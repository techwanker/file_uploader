	...
	public void invokeSetter(final String value) {
		...
		try {
			if (value == null) {
				...
				throw new IllegalStateException(
						"No implementation for parsing values type: '" + propertyType.getName() + "'");
			}
		} catch (final Exception e) {
			throw new IllegalStateException("Error assigning type '" + propertyType.getName()
	...
	protected void preparePropertyAccess(final boolean throwExceptionForMissingGetter,
		...
		if (getterMethod == null || setterMethod == null) {
			...
			if (throwExceptionForMissingGetter && getterMethod == null && throwExceptionForMissingSetter
				...
				throw new IllegalStateException(
						"Property '" + propertyName + "' does not have a " + access + "getter method '"
								+ getterMethodName + "()'" + " on the bean " + getIntrospectionClass().getName());
			} else if (throwExceptionForMissingSetter && setterMethod == null) {
				throw new IllegalStateException("Property '" + propertyName + "' does not have a " + access
						+ "setter method '" + setterMethodName + "(" + propertyType.getSimpleName() + " " + propertyName
						+ ")'" + " on the bean " + getIntrospectionClass().getName());
			}
		}
	}
