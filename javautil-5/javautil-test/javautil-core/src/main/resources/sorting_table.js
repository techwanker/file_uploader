//
// taken from http://madhatted.com/2008/6/20/the-joy-of-tables-on-cows
// on 9/18/09
//
...
var SortingTable = new Class({
  ...
  options: {
    ...
    forward_sort_decoration: new Element('span', {
    	'html': ' &uarr;'
    }),
    reverse_sort_decoration: new Element('span', {
    	'html': ' &darr;'
    })
  },
  ...
  initialize: function( table, options ) {
    ...
    this.options.forward_sort_decoration = this.options.forward_sort_decoration.clone();
    this.options.reverse_sort_decoration = this.options.reverse_sort_decoration.clone();
    var isWrapperTable = this.table.getElements('tbody tr').length == 1 && 
    	this.table.getElements('tbody td').length == 1;
    this.tbody = !isWrapperTable ? this.table.getElement('tbody') :
    	this.table.getElement('tbody tbody');
  ...
  remove_column_decoration: function( header ) {
	//	var oldWidth = header.get('width_before_decoration');
	//  header.erase('width_before_decoration');
	//  if (oldWidth) {
	//    header.setStyle('width', oldWidth);
	//  } else {
	//    header.setStyle('width', '');
	//  }
  },
  
  add_column_decoration: function( header, decorationType ) {
	//	var headerWidth = header.getComputedSize().width;
	//	var styleWidth = header.getStyle('width');
	//    var oldWidth = header.get('width_before_decoration');
	//    var changeWidth = false;
	//    var decorationWidth = 0;
	//    if (styleWidth != '' && !oldWidth) {
	//  	  header.set('width_before_decoration', styleWidth)    	  
	//  	  changeWidth = true;
	//    }
	if (decorationType == 'reverse') {
	  decoration = this.options.reverse_sort_decoration;
	} else {
	  decoration = this.options.forward_sort_decoration;  
	}
    if (decoration) {
      header.adopt(decoration);
    }
	//	if (changeWidth) {	  
	//	  var decorationWidth = 0;	  
	//	  if (decoration) {
	//	    decorationWidth = decoration.getComputedSize().width;
	//	  }
	//	  var newWidth = (headerWidth + decorationWidth) + 'px';
	//	  header.setStyle('width', newWidth);
	//	}
  },
  
  sort_by_header: function( header ){
	if (header == this.options.forward_sort_decoration || 
		header == this.options.reverse_sort_decoration) {
		header = header.getParent();
	}
	this.options.reverse_sort_decoration.dispose();
	this.options.forward_sort_decoration.dispose();
    var rows = [];
  ...
  load_conversions: function() {
    this.conversions = $A([
      ...
      { matcher: /(\d{1,2})\/(\d{1,2})\/(\d{2,4})/,
        conversion_function: function( row ) {
          var cell = $(row.row.getElementsByTagName('td')[this.sort_column]).get('text');
          cell = this.conversion_matcher.exec( cell );
          return cell[3]+
                 '00'.substr(0,2-cell[1].length).concat(cell[1])+
