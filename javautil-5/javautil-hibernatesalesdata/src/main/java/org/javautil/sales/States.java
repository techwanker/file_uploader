...
public class States {
	...
	private final ArrayList<State> states = new ArrayList<State>() {
...

		{
			...
			add(new State("AMERICAN SAMOA", "AS"));
			...
			add(new State("DISTRICT OF COLUMBIA", "DC"));
			add(new State("FEDERATED STATES OF MICRONESIA", "FM"));
			add(new State("FLORIDA", "FL"));
			add(new State("GEORGIA", "GA"));
			add(new State("GUAM", "GU"));
			...
			add(new State("MARSHALL ISLANDS", "MH"));
			...
			add(new State("VIRGIN ISLANDS", "VI"));
	...
	public String getRandomStateId() {
		final int index = random.nextInt(states.size());
		final State state = states.get(index);
		return state.getId();
	}
}
