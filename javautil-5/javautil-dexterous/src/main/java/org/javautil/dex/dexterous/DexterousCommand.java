package org.javautil.dex.dexterous;

import java.util.HashMap;

import org.apache.log4j.Logger;

import org.javautil.dex.parser.ParseException;

public enum DexterousCommand {
	// @todo support grant

	BREAK, CLEAR, SET, VARIABLE, INCLUDE, EXEC, PROMPT, SELECT, SPOOL, REM, REM_SET, STORE, GET, DESCRIBE, UPDATE, DELETE,
	INSERT, SHOW, WITH, RECORD, LOG, EXCEPTION, GET_STORE, INCLUDE_BEFORE, INCLUDE_AFTER, CROSSTAB_ROW, CROSSTAB_COLUMN,
	CROSSTAB_VALUE, OUTPUT_FORMAT, DATE_FORMAT, DATA_SOURCE, WORKSHEET, RESULTSET, AUTOTRACE, BASE, TERMOUT, MAKE_PATH,
	TRACE_ID, NUMBER, DATE, STRING, PUMP_INTO, PUMP_FROM, CONNECT, EXIT, COMMIT, WHENEVER, COMMENT, CREATE, DML, RUN, ALTER,
	DROP, PURGE, TRUNCATE, BEGIN, DECLARE, BEGIN_STATEMENT, ECHO_UNTIL, GO, PASS_ALL, NO_PASS_ALL, RUN_FILE, ESCAPE_ECHO,
	PARSE_UNTIL_GO, ECHO, RUN_FILE_CONCAT, BSH, CURSOR, WHEN, GENERATE, INJECT, WRITE;

	public static final String							revision	= "$Revision: 1.1 $";

	private static HashMap<String, DexterousCommand>	mapping		= new HashMap<String, DexterousCommand>();

	static Logger										logger		= Logger.getLogger(DexterousCommand.class.getName());
	static {
		mapping.put("ALTER", ALTER);
		mapping.put("BEGIN", BEGIN);
		mapping.put("BEGIN-STATEMENT", BEGIN_STATEMENT);
		mapping.put("BREAK", BREAK);
		mapping.put("CLEAR", CLEAR);
		mapping.put("COMMIT", COMMIT);
		mapping.put("CONN", CONNECT);
		mapping.put("CONNECT", CONNECT);
		mapping.put("CREATE", CREATE);
		mapping.put("CURSOR", CURSOR);
		mapping.put("DECLARE", DECLARE);
		mapping.put("DELETE", DELETE);
		mapping.put("DESCRIBE", DESCRIBE);
		mapping.put("DROP", DROP);
		mapping.put("ECHO", ECHO);
		mapping.put("ECHO_UNTIL", ECHO_UNTIL);
		mapping.put("ECHO-UNTIL", ECHO_UNTIL);
		mapping.put("EXCEPTION", EXCEPTION);
		mapping.put("EXEC", EXEC);
		mapping.put("EXIT", EXIT);
		mapping.put("GENERATE", GENERATE);
		mapping.put("GEN", GENERATE);
		mapping.put("GET", GET);
		mapping.put("GO", GO);
		mapping.put("INCLUDE", INCLUDE);
		mapping.put("INJECT", INJECT);
		mapping.put("INSERT", INSERT);
		mapping.put("LOG", LOG);
		mapping.put("PARSE-UNTIL-GO", PARSE_UNTIL_GO);
		mapping.put("PROMPT", PROMPT);
		mapping.put("PURGE", PURGE);
		mapping.put("RECORD", RECORD);
		mapping.put("REM", REM);
		mapping.put("R", RUN);
		mapping.put("RUN", RUN);
		mapping.put("SELECT", SELECT);
		mapping.put("SET", SET);
		mapping.put("SHOW", SHOW);
		mapping.put("SPOOL", SPOOL);
		mapping.put("STORE", STORE);
		mapping.put("TRUNCATE", TRUNCATE);
		mapping.put("VAR", VARIABLE);
		mapping.put("VARIABLE", VARIABLE);
		mapping.put("WHENEVER", WHENEVER);
		mapping.put("WHEN", WHEN);
		mapping.put("WITH", WITH);
		mapping.put("WRITE", WRITE);
		mapping.put("@", RUN_FILE);
		mapping.put("[", BSH);
		mapping.put("<%", BSH);
		mapping.put("!!!", ESCAPE_ECHO);
		mapping.put("<<<", PASS_ALL);
		mapping.put(">>>", NO_PASS_ALL);

	}

	public static DexterousCommand getCommandType(final String token) throws ParseException {
		DexterousCommand returnValue = null;
		String command = null;
		if (token != null) {
			command = token.toUpperCase();

			if (command.endsWith(";")) {
				command = command.substring(0, command.indexOf(";"));
				// logger.info("command trimmed to " + command);
			}
			if (token.startsWith("--")) {
				returnValue = COMMENT;
			} else if (token.startsWith("@") && token.length() > 1) {
				returnValue = RUN_FILE_CONCAT;
			} else {
				returnValue = mapping.get(command);
			}
		}
		if (returnValue == null) {
			throw new ParseException("unknown command " + token);
		}

		return returnValue;
	}

	public static DexterousCommand getCommandType(final String[] tokens) {
		DexterousCommand returnValue = null;
		String command = null;
		if (tokens == null) {
			throw new IllegalArgumentException("tokens is null");
		}
		if (tokens.length == 0) {
			return null;
		}
		if (tokens.length > 0) {
			command = tokens[0].toUpperCase();

			if (command.endsWith(";")) {
				command = command.substring(0, command.indexOf(";"));
				// logger.info("command trimmed to " + command);
			}
			if (tokens[0].startsWith("--")) {
				returnValue = COMMENT;
			} else {
				returnValue = mapping.get(command);
				if (returnValue == REM) {
					if (tokens.length > 1) {
						final DexterousCommand cmd = mapping.get(tokens[1].toUpperCase());

						if (cmd == SET) {
							returnValue = REM_SET;
						}
					}

				}
			}
		}
		if (returnValue == null) {
			throw new IllegalArgumentException("unknown command " + tokens[0]);
		}
		return returnValue;
	}

	public boolean isSQL() {
		return isDML() || isDDL();
	}

	@SuppressWarnings("incomplete-switch")
	public boolean isDML() {
		boolean  retval = false;
		switch (this) {
		case SELECT:
		case INSERT:
		case UPDATE:
		case DELETE:
		case WITH:
			retval = true;
		}
		return retval;
	}

	@SuppressWarnings("incomplete-switch")
	public boolean isDDL() {
		boolean retval = false;
		switch (this) {
		case ALTER:
		case CREATE:
		case DROP:
		case TRUNCATE:
			retval = true;

		}
		return retval;
	}
}
