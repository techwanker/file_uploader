...
//import com.dbexperts.dex.DbexpertsEnvironment;
//import com.dbexperts.io.MimeType;
//import com.dbexperts.misc.BuildInfo;
//import com.dbexperts.text.MessageBuilder;
// TODO Make fully declarative
public class DexterousArguments {

	public static final String revision = "$Revision: 1.1 $";

	public static final String DEXTEROUS_LOG_MASK = "%f.%d.xml";
	...
	private String stickyViolationPolicy = "abort";
	...
	private Level fileLoggerLevel  = Level.WARN;

	private Level stdoutLoggerLevel = Level.WARN;


//	private final StringParam logFileMaskOption = new StringParam("logFileNameMask","mask for file name ");
//
//	private final FileParam logDirectoryOption = new FileParam("logDirectory", "directory in which log files are to be created",
...
//	// Arguments
//	//
//	private final FileParam inputFilesArg = new FileParam("file", "files to be processed",
...
//
//	private final StringParam dataSourceOption = new StringParam("dataSource",
//	"name of the dataSource");
//
//	private final StringParam promptOption = new StringParam("prompt","the prompt text");
//
//	private final StringParam userNameOption = new StringParam("username","username or username/password");
//	/**
//	 * The option for defining the requirements for the configuration file.
//	 */
//	private final FileParam   configFileOption = new FileParam("config","configuration file ",
//			FileParam.IS_FILE & FileParam.IS_READABLE,
//			Parameter.OPTIONAL,
//			Parameter.SINGLE_VALUED);
//
//	private final FileParam stickyCommandFilesOption = new FileParam("stickyCommands","file containing commands that may not be over-ridden in script files ",
//			FileParam.IS_FILE & FileParam.IS_READABLE,Parameter.OPTIONAL,Parameter.MULTI_VALUED);
//
//	private final FileParam defaultBaseDirectoryOption = new FileParam("baseDirectory", "base directory for spooled output",
//			FileParam.IS_DIR & FileParam.IS_WRITEABLE,
//			Parameter.OPTIONAL, Parameter.SINGLE_VALUED);
//
//
//	private final String[] logLevels = {"SEVERE","WARNING","INFO","FINE","FINER","FINEST",
//			"severe","warning","info","fine","finer","finest",
//	};
//	private final StringParam stdoutLogLevelOption = new StringParam("stdoutLoggerLevel","stdout logging level ",logLevels);
//	private final StringParam fileLogLevelOption = new StringParam("fileLoggerLevel", "log level for log file", logLevels);
//	private final StringParam mimeTypeOption = new StringParam("mimeType","default mime type ");
//
//	private final String[] stickyViolationPolicies = new String[] {"ignore","warn","abort"};
//	private final StringParam stickyViolationPolicyOption = new StringParam("stickyViolationPolicy",
//			"action to take when a sticky rule is violated",
//			stickyViolationPolicies, // restricted values
//			Parameter.OPTIONAL,
//			Parameter.SINGLE_VALUED);

//	private final BooleanParam verifyOnlyOption = new BooleanParam("verifyOnly","verify that the script will work but set select timeout to 1 ms");
//
//	private final BooleanParam disableCacheOption = new BooleanParam("disableCache","disable Cache default " + disableCache);
//
//	private final BooleanParam echoOption = new BooleanParam("echo","echo input");
//
//	private final BooleanParam logTimeOption = new BooleanParam("logTime","timestamp log messages");
//	private final BooleanParam logMethodOption = new BooleanParam("logMethod","log method and class");
//	private final BooleanParam logMethodOnlyOption = new BooleanParam("logMethodOnly", "log the method name but not the class name that generated the message");
//
...
//			promptOption, userNameOption, configFileOption, echoOption, stdoutLogLevelOption,
//			disableCacheOption, mimeTypeOption, defaultBaseDirectoryOption, stickyViolationPolicyOption, stickyCommandFilesOption, eventOption, verifyOnlyOption,
//			logTimeOption, logMethodOption, logMethodOnlyOption, logDirectoryOption, logFileMaskOption};
//
//	private final BuildInfo bi = new BuildInfo();
//	private final CmdLineHandler cl = new VersionCmdLineHandler(bi.getVersion(),
//			new HelpCmdLineHandler(helpText, "dexterous", "dexter command line interface",
//					options,
//					arguments));//
//
//
...
//			if (logDir != null) {
//				logger.info("using environment variable " + DbexpertsEnvironment.DEXTEROUS_LOG_DIR);
//				final File logDirFile = new File(logDir);
//				if (! logDirFile.canWrite()) {
//					throw new IllegalArgumentException("can't write to " + logDirFile.getAbsolutePath());
//				}
...
//		final String helpResource = "com.dbexperts/dex/DexterousHelp.txt";
//		final StringBuffer buff = new StringBuffer();
//		BufferedReader in;
//		try {
//			final InputStream stream = getClass().getClassLoader().getResourceAsStream(helpResource);
//			if (stream == null) {
//				throw new java.lang.IllegalArgumentException("irresolvable resource " + helpResource);
//			}
...
//
//		prompt = promptOption.getValue() == null ? "dex> " : promptOption.getValue();
...
//					throw new IllegalArgumentException("event number '" + s + "' cannot be parsed as an integer" );
//				}
//			}
//
//		}
...
//
//	/**
//	 * @param defaultBaseDirectory the defaultBaseDirectory to set
//	 */
...
//
//	/**
//	 * @param defaultMimeType the defaultMimeType to set
//	 */
...
//
//	/**
//	 * @param dieOnParseError the dieOnParseError to set
//	 */
...
//	}
//
//	// todo write a cli help
////	private void startHelp() throws IOException {
////		File temp = new File("/tmp/index.html");
////		BufferedWriter bw = new BufferedWriter(new FileWriter(temp));
////		String helpResource = "com.dbexperts/dex/webhelp/Index.txt";
////		StringBuffer buff = new StringBuffer();
////		BufferedReader in;
////		try {
////			InputStream stream = getClass().getClassLoader().getResourceAsStream(helpResource);
////			if (stream == null) {
////				throw new java.lang.IllegalArgumentException("irresolvable resource " + helpResource);
////			}
////			in = new BufferedReader(new InputStreamReader(stream, "8859_1"));
