...
//		final DefaultResourceLoader resolver = new DefaultResourceLoader(TestData.class.getClassLoader());
	...
	//	final InputStream inputStream = resolver.getResource(pathName).getInputStream();
	    InputStream inputStream = getClass().getClassLoader().getResourceAsStream(pathName);
		
		logger.debug("testing against " + fileName);
		File file = new File(pathName);

		System.out.println("test file path is: " + file.getAbsolutePath());
		if (inputStream == null) {
	...
	public void validateResult(final StackTraceElement invoker, final byte[] testArray) {
		try {
			...
			Assert.assertEquals("Result size is different from expected result size.", validArray.length,
					testArray.length);

			Assert.assertTrue("Result bytes are different from expected result bytes.",
