...
import com.javautil.util.StringHelper;
...
public class PropertyManagerFile {
    ...
    //private static final String className = "com.javautil.property.PropertyManagerFile";
    private static final Logger logger = Logger.getLogger(PropertyManagerFile.class.getName());
    ...
    /**
     Strip all trailing spaces tabs and the line continuation character from a string.

     If the string ends in a <code>\</code> followed by any number of spaces and tabs
     in any order, the continuation character and the following tabs and spaces are
    ...
    /*
     * Returns true if the given line ends in a line continuation character <code>\</code>.
     * Should strip trailing white characters !!!
     */
     ...
     * Constructor.
     */
    public PropertyManagerFile(String fileName)
     ...
     */
    public synchronized void read(InputStream inStream)
            throws IOException {
        ...
        if (line.length() > 0) {
            ...
            if ((firstChar != '#') && (firstChar != '!')) {
                while (continueLine(line)) {
                    ...
                    int firstNonWhite = StringHelper.firstNotIn(line, " \t");
                    if (firstNonWhite > -1) {
