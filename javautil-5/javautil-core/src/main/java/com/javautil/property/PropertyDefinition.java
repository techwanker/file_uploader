...
class PropertyDefinition {
        ...
        /**
        Name of the property.  Typically the fully qualified className that uses the property.
        */
    ...
    /*
        ...
        */
        ...
        public PropertyDefinition (String text)
        {
                ...
                while ((word = getToken()) != null) {
                        //System.out.println("token was " + word);
                        Integer wordNbr = (Integer) reservedWords.get(word);
                        if (wordNbr == null) {
                                if (word.startsWith("default")) {
                                        wordNbr = (Integer) reservedWords.get("default");
                                }
                                else {
                                        throw new java.lang.IllegalArgumentException("invalid keyword '" + word + "'");
                                }
                        }
                        //System.out.println("word '" + word + "'" + wordNbr);
     ...
     boolean error = false;
        ...
        public void required(Boolean val) {
                if (required != null) {
                        throw new java.lang.IllegalStateException("attempted to set 'required','mandatory' or 'optional' more than once or conflict");
                }
                required = val;
        }
        ...
        private String getToken() {
                ...
                int tokenEnd;
                //
                tokenStart = StringHelper.firstNotIn(text," \t\r\n\f",tokenPos);
                if (tokenStart > -1) {
                        tokenEnd   = StringHelper.indexOfAny(text," \t\r\n\f",tokenStart);
                        if(tokenEnd == -1)
                        {
                ...
                /*
                if (rc != null) {
                        System.out.println("$$$$$$ returning token '" + rc + "'");
                } else {
