...
public class FileNode {
    ...
    public FileNode(File file, FileNode parent) {
        logger.fine("file: " + file + " parent " + parent);
        this.file = file;
        refresh();
    }
    ...
    public FileNode(String fileName, FileNode parent) {
        ...
        logger.fine("fileName: " + fileName + " parent " + parent);
        this.file = new File(fileName);
        refresh();
    }

    /**
     Adds a child node after ensuring that the child is not the parent or any ancestor
     of the parent.
    ...
    public void addChild(File file)
        ...
        for (FileNode ancestor = this; ancestor != null; ancestor = ancestor.parent) {
            if (ancestor.file.getCanonicalFile().equals(file)) {
                throw new java.lang.IllegalStateException("Attempt to add " + file +
                        " as child to " + this.file +
    ...
    /**
     Returns true if this node or any of its children, recursively,
     have a modified date stamp different from the file modification time.
     */
    ...
    /**
     ...
     The logic here is that somebody tried to update the timestamp with unix touch command
     or something similar and messed up.
    ...
    private FileNode[] modifiedNodes() {
        ...
        // HashMap is used to throw away the duplicate reporting of a node generated during recursive processing
