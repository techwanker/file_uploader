...
public class SqlStatementRunner {
	...
	public SqlStatementRunner(Connection connection, Dialect dialect,SqlStatements sqlStatements,
			int verbosity) {
	...
	public void process(Binds binds) throws SQLException {
		for (SqlStatement stmt : sqlStatements) {
			...
			if (verbosity > 0) {
				logger.info(stmt.getName() + " records " + updateCount + 
						" millis " + t.getElapsedMillis());
			}
		}
